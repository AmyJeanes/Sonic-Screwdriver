...
Run from the project root. The `.` is required (no-arg fails on Windows), and the working directory must be the Sonic-Screwdriver root so `.luarc.json`'s relative paths resolve.
